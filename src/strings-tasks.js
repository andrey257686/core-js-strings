...
function isPalindrome(str) {
  const strLow = str.toLowerCase().replace(/\W/g, '');
  return strLow.split('').reverse().join('') === strLow;
  // throw new Error('Not implemented');
}
...
function encodeToRot13(str) {
  ...
  let res = '';
  for (let i = 0; i < arr.length; i += 1) {
    const index = inputRot13.indexOf(arr[i]);
    res += index === -1 ? arr[i] : outputRot13[index];
  }
  return res;
  // throw new Error('Not implemented');
}
